...
人間向けレビューHTMLは`human/runs/<runId>/`へ生成する。共通CSS・テンプレートは`human/styles/`と`human/templates/`を使い、canonical AI artifactは`ai/runs/<runId>/`、runtime evidenceは`.agent-runs/<runId>/`へ保存する。docsへの新規書込は禁止し、既存docsはlegacy read-onlyとして扱う。計画資料とレビューHTMLはGit管理するが、秘密情報や大容量runtime evidenceはGitに含めない。外部共有は初版の対象外であり、PR本文からローカルHTMLをGitHubで閲覧可能とは案内しない。
